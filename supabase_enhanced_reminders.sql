-- Enhanced License Reminder System
-- Updates for 60, 30, 15, 7, 1 day reminders + daily after expiration
-- Run this in Supabase SQL Editor

-- Update the email_reminders table to support new reminder types
ALTER TABLE email_reminders 
DROP CONSTRAINT IF EXISTS email_reminders_reminder_type_check;

ALTER TABLE email_reminders 
ADD CONSTRAINT email_reminders_reminder_type_check 
CHECK (reminder_type IN ('60_days', '30_days', '15_days', '7_days', '1_day', 'overdue_daily'));

-- Create enhanced view for licenses needing reminders
...
-- Create a view for upcoming expirations with enhanced date ranges
...
-- Create a helpful view for overdue licenses
...
-- SELECT * FROM licenses_needing_reminders;
-- SELECT * FROM upcoming_expirations WHERE status_category IN ('critical', 'warning');
-- SELECT * FROM overdue_licenses;
