#!/usr/bin/env python3
"""
License Reminder System with EmailJS Integration
Uses EmailJS free service for sending emails
"""
...
class LicenseReminderSystemEmailJS:
    def __init__(self):
        """Initialize the License Reminder System with EmailJS"""
        ...
        self.emailjs_user_id = os.getenv('EMAILJS_USER_ID')  # Also called Public Key
        self.emailjs_private_key = os.getenv('EMAILJS_PRIVATE_KEY')  # Optional for security
        ...
        try:
            ...
            logger.info("License Reminder System with EmailJS initialized successfully")
    ...
    def upload_excel_to_supabase(self) -> bool:
        ...
        try:
            ...
            df = df.dropna(subset=['LIC_ID'])  # Remove rows without license ID (but we won't store LIC_ID)
            ...
            # Remove lic_id column since we'll use database auto-generated id instead
            if 'lic_id' in df.columns:
                df = df.drop('lic_id', axis=1)
            ...
            # Since we removed lic_id, we'll use lic_name as a unique identifier for updates
            # First, get existing records to check for duplicates by name
            existing_result = self.supabase.table('licenses').select('id, lic_name').execute()
            existing_names = {row['lic_name']: row['id'] for row in existing_result.data}
            
            # Separate new records from existing ones based on lic_name
            new_records = []
            existing_records = []
            
            for record in records:
                if record.get('lic_name') in existing_names:
                    record['id'] = existing_names[record['lic_name']]
            ...
            for record in existing_records:
                ...
                self.supabase.table('licenses').update(record).eq('id', record_id).execute()
    ...
    def get_upcoming_expirations(self) -> List[Dict]:
        """Get all upcoming expirations (next 35 days)"""
        try:
            result = self.supabase.table('upcoming_expirations').select('*').execute()
            return result.data
        except Exception as e:
            logger.error(f"Error fetching upcoming expirations: {str(e)}")
            return []
    ...
    def create_email_content(self, license_data: Dict, reminder_type: str) -> tuple:
        ...
        body = f"""Dear {license_data['lic_name']},
...
"""
    ...
    def send_email_via_emailjs(self, to_emails: List[str], subject: str, body: str) -> bool:
        """Send email using EmailJS service"""
        if not self.email_enabled:
            logger.warning("Email sending disabled - EmailJS not configured")
            return False
        ...
        try:
            ...
            if self.emailjs_private_key:
                email_data['accessToken'] = self.emailjs_private_key
            
            # Send request to EmailJS
            headers = {
                'Content-Type': 'application/json',
                'User-Agent': 'License-Reminder-System/1.0'
            }
            
            response = requests.post(url, data=json.dumps(email_data), headers=headers)
            
            if response.status_code == 200:
                logger.info(f"Email sent successfully via EmailJS to: {', '.join(to_emails)}")
                return True
            else:
            ...
            logger.error(f"Error sending email via EmailJS to {', '.join(to_emails)}: {str(e)}")
            return False
        ...
        for license_data in licenses:
            try:
                ...
                if not email_addresses:
                    logger.warning(f"No valid email addresses for license {license_data['id']}")
                    continue
                ...
                # Send email
                email_sent = self.send_email_via_emailjs(email_addresses, subject, body)
                ...
                logger.error(f"Error processing reminder for license {license_data.get('id')}: {str(e)}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python license_reminder_emailjs.py upload    - Upload Excel data to Supabase")
        print("  python license_reminder_emailjs.py check     - Run reminder check once")
        print("  python license_reminder_emailjs.py schedule  - Start daily scheduler")
        print("  python license_reminder_emailjs.py test      - Test EmailJS configuration")
        return
    
    command = sys.argv[1].lower()
    
    try:
        system = LicenseReminderSystemEmailJS()
        ...
        if command == 'upload':
            ...
            if system.email_enabled:
                test_emails = ['test@example.com']
                success = system.send_email_via_emailjs(
                    test_emails,
                    "Test Email from License Reminder System",
                    "This is a test email to verify EmailJS configuration."
                )
                if success:
                    logger.info("EmailJS test successful!")
                else:
                    logger.error("EmailJS test failed!")
            else:
                logger.error("EmailJS not configured!")
                
        else:
            print(f"Unknown command: {command}")
            print("Use 'upload', 'check', 'schedule', or 'test'")
